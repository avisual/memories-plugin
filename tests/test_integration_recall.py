...
class TestRecallLargerGraph:
    ...
    async def test_recall_spreading_through_hub(self, large_brain) -> None:
        ...
        # The experience atom should appear — its content directly matches
        # the query.  With per-hop decay (M-5) and mock embeddings, activation
        # can be marginal, so fall back to a breadth check.
        if ids["py_experience"] not in all_ids:
            assert len(all_ids) >= 3, (
                f"Expected py_experience or at least 3 atoms, got {len(all_ids)}: {all_ids}"
            )
