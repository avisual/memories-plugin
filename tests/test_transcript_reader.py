...
def _mock_brain(
    ...
    brain._learning.assess_novelty = AsyncMock(return_value=(is_novel, 0))
...
class TestStoreTranscriptInsights:
    ...
    async def test_mixed_novel_and_non_novel(self) -> None:
        ...
        async def alternating_novelty(content: str) -> tuple[bool, int]:
            nonlocal call_count
            call_count += 1
            return call_count % 2 == 1, 0  # odd calls are novel
...
class TestExtractAtomicFacts:
    ...
    async def test_strips_bullet_prefix(self) -> None:
        ...
        mock_response = MagicMock()
        mock_response.response = "- The key insight is that X implies Y through transitive dependency resolution."
        mock_client = AsyncMock()
        mock_client.generate = AsyncMock(return_value=mock_response)
    ...
    async def test_caps_at_five_facts(self) -> None:
        ...
        lines = "\n".join(f"- Fact number {i} about the database connection pool with WAL mode and thread safety." for i in range(8))
        mock_response = MagicMock()
    ...
    async def test_extraction_called_when_flag_set(self) -> None:
        ...
        brain = _mock_brain(is_novel=True, deduplicated=False)
        facts = [
            "First atomic fact about the database connection pooling strategy in storage.py module.",
            "Second atomic fact about the synapse pruning threshold being set to 0.1 by default.",
        ]
