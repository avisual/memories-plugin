...
class TestReclassifyMisclassifiedAntipatterns:
    ...
    async def test_long_antipattern_without_prescriptive_keywords_reclassified(
        self, storage, mock_embeddings
    ) -> None:
        """Antipatterns lacking prescriptive keywords should be reclassified
        regardless of length — the fix targets content quality, not length.
        """
        long_content = "x" * 150  # exactly 150 chars, no prescriptive keywords
        aid = await insert_atom(
            storage,
            long_content,
            atom_type="antipattern",
        ...
        rows = await storage.execute("SELECT type FROM atoms WHERE id = ?", (aid,))
        assert rows[0]["type"] == "experience", (
            "Antipattern without prescriptive keywords should be reclassified "
            "to experience regardless of length"
        )

    async def test_long_genuine_antipattern_not_reclassified(
        self, storage, mock_embeddings
    ) -> None:
        """Genuine antipatterns with prescriptive keywords are kept."""
        content = (
            "You should never use eval() on user input because it allows "
            "arbitrary code execution. This is a common security vulnerability "
            "that can compromise the entire server. " + "x" * 50
        )
        aid = await insert_atom(
            storage,
            content,
        ...
        rows = await storage.execute("SELECT type FROM atoms WHERE id = ?", (aid,))
        assert rows[0]["type"] == "antipattern", (
            "Genuine antipattern with 'should never' should NOT be reclassified"
        )
