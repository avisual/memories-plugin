...
class TestSubagentStartHook:
    ...
    @pytest.mark.asyncio
    async def test_subagent_start_does_not_store_atom(self, tmp_path):
        """Subagent-start tracks stats but does NOT store delegation atoms (noise)."""
        from memories.cli import _hook_subagent_start
        ...
        with patch("memories.cli._get_brain", return_value=brain_mock):
            with patch("memories.cli._record_hook_stat", new_callable=AsyncMock) as mock_stat:
                data = {
                    "agent_type": "Explore",
                    "agent_id": "agent-123",
        ...
        assert not stored, "subagent-start should NOT store atoms — delegation info is noise"
        mock_stat.assert_called_once()

    @pytest.mark.asyncio
    async def test_bash_agent_type_skipped(self, tmp_path):
        """Bash agent type is skipped entirely — no stats, no storage."""
        from memories.cli import _hook_subagent_start

        brain_mock = MagicMock()

        with patch("memories.cli._get_brain", return_value=brain_mock) as mock_brain:
            with patch("memories.cli._record_hook_stat", new_callable=AsyncMock) as mock_stat:
                data = {
                    "agent_type": "Bash",
                    "agent_id": "agent-456",
...
class TestZeroTimestampEdgeCase:
    ...
    async def test_zero_epoch_atom_gets_half_increment(self, tmp_path):
        ...
        updated = sum(await mgr.hebbian_update([30, 31], atom_timestamps=atom_timestamps))
    ...
    async def test_both_zero_epoch_atoms_get_full_increment(self, tmp_path):
        ...
        updated = sum(await mgr.hebbian_update([40, 41], atom_timestamps=atom_timestamps))
        assert updated >= 1
