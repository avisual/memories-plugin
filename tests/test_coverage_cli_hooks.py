...
class TestFormatPathways:
    ...
    def test_formats_pathways(self):
        ...
        # W16a: strength no longer shown; content preview or ID fallback used.
        assert "1" in lines[1] or '"' in lines[1]
...
class TestHookBudget:
    ...
    def test_session_start_gets_larger_budget(self):
        # W16a: session-start uses background tier; prompt-submit with a
        # medium-length prompt also uses background, so compare against a
        # short prompt (minimal tier).
        budget = _hook_budget("session-start")
        assert budget > _hook_budget("prompt-submit", prompt_length=50)

    def test_pre_tool_gets_smallest_budget(self):
        # W16a: pre-tool is always minimal, prompt-submit with a long
        # prompt uses critical tier which is strictly larger.
        budget = _hook_budget("pre-tool")
        assert budget < _hook_budget("prompt-submit", prompt_length=600)
...
class TestHookPromptSubmit:
    ...
    async def test_prompt_submit_with_project_triple_recall(self):
        """With a project, prompt-submit runs triple recall (project + general + cross-project antipatterns)."""
        mock_brain = _make_mock_brain(atoms=[
        ...
        # Should call recall 3 times (project + general + cross-project antipatterns).
        assert mock_brain.recall.await_count == 3
        assert "[memories]" in result
    ...
    async def test_prompt_submit_no_atoms_returns_no_results_signal(self):
        """W16a: When no atoms recalled, returns XML-wrapped 'no results' message."""
        mock_brain = _make_mock_brain(atoms=[])
        ...
        assert "<memories>" in result
        assert "No relevant prior learnings" in result
        assert "</memories>" in result
