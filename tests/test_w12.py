...
class TestPostToolFailureHook:
    ...
    @pytest.mark.asyncio
    async def test_skip_tool_on_failure_respected(self):
        """Tools in _SKIP_TOOLS_ON_FAILURE must be silently ignored."""
        from memories.cli import _hook_post_tool_failure

        data = {
            "tool_name": "TaskCreate",
            "error": "Some task creation error that is long enough to pass the 20 char threshold",
            "tool_input": {},
            "session_id": "s1",
        }
        result = await _hook_post_tool_failure(data)
    ...
    @pytest.mark.asyncio
    async def test_edit_constraint_captured_as_antipattern(self, tmp_path):
        """Edit 'File has not been read yet' must be captured as antipattern."""
        from memories.cli import _hook_post_tool_failure
        ...
        with patch("memories.cli._get_brain", return_value=brain_mock):
            with patch("memories.cli._save_hook_atoms", new_callable=AsyncMock):
                with patch("memories.cli._record_hook_stat", new_callable=AsyncMock):
                    data = {
                        "tool_name": "Edit",
                        "error": "File has not been read yet. You must read the file before editing it.",
        ...
        assert stored_calls, "Expected remember() to be called for Edit constraint error"
        assert stored_calls[0]["type"] == "antipattern"
        assert "Avoid" in stored_calls[0]["content"]

    @pytest.mark.asyncio
    async def test_edit_not_unique_captured(self, tmp_path):
        """Edit 'old_string is not unique' must be captured (not skipped)."""
        from memories.cli import _hook_post_tool_failure
        ...
        with patch("memories.cli._get_brain", return_value=brain_mock):
            with patch("memories.cli._save_hook_atoms", new_callable=AsyncMock):
                with patch("memories.cli._record_hook_stat", new_callable=AsyncMock):
                    data = {
                        "tool_name": "Edit",
                        "error": "The old_string is not unique in the file. Provide more context.",
                        "tool_input": {"file_path": "/some/file.py"},
                        "session_id": "test-session",
                        "cwd": str(tmp_path),
                    }
        ...
        assert stored_calls, "Expected remember() to be called for ambiguous Edit"
        assert "Avoid" in stored_calls[0]["content"]
...
class TestTemporalHebbian:
    ...
    async def test_recent_pairs_get_full_increment(self, tmp_path):
        ...
        updated = sum(await mgr.hebbian_update([1, 2], atom_timestamps=atom_timestamps))
    ...
    async def test_distant_pairs_get_half_increment(self, tmp_path):
        ...
        updated = sum(await mgr.hebbian_update([10, 11], atom_timestamps=atom_timestamps))
        assert updated >= 1
    ...
    async def test_missing_timestamps_fallback_to_full_increment(self, tmp_path):
        ...
        updated = sum(await mgr.hebbian_update([20, 21], atom_timestamps=None))
        assert updated >= 1
