...
class TestSynapseManagerHebbian:
    ...
    async def test_strengthens_existing_synapse(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        count = sum(await mgr.hebbian_update([a1, a2]))

        assert count == 1
        updated = await mgr.get(sid)
        assert updated is not None
        # Default hebbian_increment is 0.12; BCM formula: 0.5 + 0.12 * (1 - 0.5) = 0.56
        assert updated.strength == pytest.approx(0.56, abs=1e-6)
    ...
    async def test_deduplicates_atom_ids(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        count = sum(await mgr.hebbian_update([a1, a2, a1, a2, a1]))
...
class TestHebbianBatchUpdate:
    ...
    async def test_batch_strengthens_existing_synapses(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        count = sum(await mgr.hebbian_update([a1, a2, a3]))

        assert count == 3

        # Verify each synapse was strengthened by hebbian_increment (0.12).
    ...
    async def test_deduplicates_input_ids(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        count = sum(await mgr.hebbian_update([a1, a1, a2, a2]))
        ...
        syn = await mgr.get(sid)
        assert syn is not None
        # Strengthened exactly once with BCM formula: 0.5 + 0.12 * (1 - 0.5) = 0.56, not twice.
        assert syn.strength == pytest.approx(0.56, abs=1e-6)
...
class TestHebbianCorrectedLearning:
    ...
    async def test_strengthens_all_synapses_for_pair(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        count = sum(await mgr.hebbian_update([a, b]))
    ...
    async def test_bcm_formula_numerically(self, storage) -> None:
        ...
        mgr = SynapseManager(storage)
        increment = mgr._cfg.learning.hebbian_increment  # default 0.12
