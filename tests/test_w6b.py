...
class TestPruneDormantSynapses:
    ...
    async def test_dormant_synapse_strength_decayed(self, storage: Storage) -> None:
        ...
        # Passive synapse decay applies first (0.97 base rate for related-to),
        # then dormant decay: 0.5 * 0.97 = 0.485, then 0.485 * 0.80 = 0.388
        # Just check it went below the original 0.5
        assert len(rows) == 1
    ...
    async def test_dormant_synapse_below_threshold_deleted(self, storage: Storage) -> None:
        ...
        # After passive decay: 0.063 * 0.97 = 0.06111 (above prune_threshold 0.05)
        # After dormant decay: MAX(0.05, 0.06111 * 0.80) = MAX(0.05, 0.0489) = 0.05
    ...
    async def test_recently_created_dormant_not_affected(self, storage: Storage) -> None:
        ...
        assert len(rows) == 1
        # Only passive synapse decay applies: 0.5 * 0.97 = 0.485
        # If dormant decay also fired: 0.485 * 0.80 = 0.388
    ...
    async def test_activated_synapse_not_affected(self, storage: Storage) -> None:
        ...
        assert len(rows) == 1
        # Only passive decay applies: 0.5 * 0.97 = 0.485
        # If dormant decay also fired: 0.485 * 0.80 = 0.388
...
class TestPruneStaleWarnsAgainst:
    ...
    async def test_warns_against_above_strength_threshold_preserved(self, storage: Storage) -> None:
        ...
        # After passive decay: 0.5 * 0.97 = 0.485 which is still >= 0.4
        assert len(rows) == 1
...
class TestDecayFactor:
    ...
    def test_decay_factor_default_is_085(self) -> None:
        """The default decay_factor should be 0.85 (raised from 0.7 for better hop2 activation)."""
        cfg = RetrievalConfig()
