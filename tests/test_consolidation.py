...
class TestAtomDecay:
    ...
    async def test_decay_reduces_confidence(self, storage: Storage) -> None:
        ...
        # Default decay_rate is 0.97; "fact" type has multiplier 0.995 (B1).
...
class TestSynapseDecay:
    ...
    async def test_synapse_strength_reduced(self, storage: Storage) -> None:
        ...
        Non-related-to synapses use the base decay rate (0.97).
        """
        ...
        # Use "elaborates" to test base decay rate
        ...
        # After decay: 0.6 * 0.97 = 0.582.
        assert len(rows) == 1
        assert abs(rows[0]["strength"] - 0.6 * 0.97) < 1e-4

    async def test_related_to_synapse_decays_at_base_rate(self, storage: Storage) -> None:
        """Related-to synapses now decay at the same base rate as typed synapses.

        The former 0.9 penalty was removed — the monoculture it addressed
        (99.7% related-to) is now at 91.5%, and the penalty was destroying
        the graph when consolidation ran multiple times per day.
        """
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        ...
        # Both decay at base rate: 0.6 * 0.97 = 0.582
        assert len(rows_related) == 1
        assert abs(rows_related[0]["strength"] - 0.6 * 0.97) < 1e-4

        assert len(rows_elaborates) == 1
        assert abs(rows_elaborates[0]["strength"] - 0.6 * 0.97) < 1e-4
    ...
    async def test_weak_synapse_pruned_after_decay(self, storage: Storage) -> None:
        ...
        # Prune threshold default is 0.05.  0.04 * 0.97 = 0.0388 < 0.05.
...
# -----------------------------------------------------------------------
# 3. Time-proportional synapse decay
# -----------------------------------------------------------------------


class TestTimeProportionalDecay:
    """Verify time-proportional decay computes correct factors."""

    async def test_first_run_uses_full_day_fraction(self, storage: Storage) -> None:
        """First decay (no metadata) uses elapsed_fraction=1.0 — same as legacy."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        syn_id = await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)

        engine = _make_engine(storage)
        await engine.reflect()

        rows = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_id,))
        # First run: elapsed_fraction=1.0, so 0.97^1.0 = 0.97, strength = 0.6 * 0.97
        assert abs(rows[0]["strength"] - 0.6 * 0.97) < 1e-4

    async def test_half_day_elapsed_applies_half_decay(self, storage: Storage) -> None:
        """Decay after 12 hours uses elapsed_fraction=0.5 — sqrt(0.97)."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        syn_id = await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)

        # Set last decay to 12 hours ago
        twelve_hours_ago = (datetime.now(tz=timezone.utc) - timedelta(hours=12)).isoformat()
        await storage.set_metadata("last_synapse_decay_at", twelve_hours_ago)

        engine = _make_engine(storage)
        await engine.reflect()

        rows = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_id,))
        expected = 0.6 * (0.97 ** 0.5)  # ~0.6 * 0.9849 = ~0.5909
        assert abs(rows[0]["strength"] - expected) < 1e-3

    async def test_two_half_day_runs_equal_one_full_day(self, storage: Storage) -> None:
        """Two runs 12h apart compound to the same total as one 24h run."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        syn_id = await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)

        # First run: 12h elapsed — force the rate limiter to allow decay
        # by inserting an old consolidation_log entry.
        twelve_hours_ago = (datetime.now(tz=timezone.utc) - timedelta(hours=12)).isoformat()
        await storage.set_metadata("last_synapse_decay_at", twelve_hours_ago)
        await storage.execute_write(
            "INSERT INTO consolidation_log (action, created_at) VALUES ('reflect', '2020-01-01 00:00:00')"
        )
        ...
        rows = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_id,))
        after_first = rows[0]["strength"]
        # Verify first half-day decay was applied
        assert abs(after_first - 0.6 * (0.97 ** 0.5)) < 1e-3

        # Second run: another 12h elapsed. We must backdate both the metadata
        # timestamp AND the consolidation_log so the rate limiter allows decay again.
        twelve_hours_ago2 = (datetime.now(tz=timezone.utc) - timedelta(hours=12)).isoformat()
        await storage.set_metadata("last_synapse_decay_at", twelve_hours_ago2)
        await storage.execute_write(
            "UPDATE consolidation_log SET created_at = '2020-01-01 00:00:00'"
        )

        engine2 = _make_engine(storage)
        await engine2.reflect()

        rows2 = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_id,))
        after_second = rows2[0]["strength"]

        # Two half-day decays compound to exactly one full day:
        # 0.6 * 0.97^0.5 * 0.97^0.5 = 0.6 * 0.97^1.0
        expected = 0.6 * 0.97
        assert abs(after_second - expected) < 1e-3

    async def test_elapsed_fraction_capped_at_3_days(self, storage: Storage) -> None:
        """Even after 10 days offline, decay is capped at 3 days."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        syn_id = await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)

        # Set last decay to 10 days ago
        ten_days_ago = (datetime.now(tz=timezone.utc) - timedelta(days=10)).isoformat()
        await storage.set_metadata("last_synapse_decay_at", ten_days_ago)

        engine = _make_engine(storage)
        await engine.reflect()

        rows = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_id,))
        # Capped at 3 days: 0.6 * 0.97^3.0
        expected = 0.6 * (0.97 ** 3.0)
        assert abs(rows[0]["strength"] - expected) < 1e-3

    async def test_metadata_timestamp_updated_after_decay(self, storage: Storage) -> None:
        """After decay, the metadata timestamp should be updated to near-now."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        await _insert_synapse(storage, a1, a2, strength=0.6)

        engine = _make_engine(storage)
        await engine.reflect()

        val = await storage.get_metadata("last_synapse_decay_at")
        assert val is not None
        ts = datetime.fromisoformat(val)
        if ts.tzinfo is None:
            ts = ts.replace(tzinfo=timezone.utc)
        assert (datetime.now(tz=timezone.utc) - ts).total_seconds() < 60

    async def test_encoded_with_decays_faster_proportionally(self, storage: Storage) -> None:
        """encoded-with uses decay_rate * 0.95 per day, also time-proportional."""
        a1 = await _insert_atom(storage, content="atom 1", created_at="2020-01-01 00:00:00")
        a2 = await _insert_atom(storage, content="atom 2", created_at="2020-01-01 00:00:00")
        a3 = await _insert_atom(storage, content="atom 3", created_at="2020-01-01 00:00:00")
        syn_related = await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)
        syn_encoded = await _insert_synapse(storage, a1, a3, relationship="encoded-with", strength=0.6)

        engine = _make_engine(storage)
        await engine.reflect()

        rows_r = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_related,))
        rows_e = await storage.execute("SELECT strength FROM synapses WHERE id = ?", (syn_encoded,))

        # First run: fraction=1.0
        # related-to: 0.6 * 0.97^1.0 = 0.582
        # encoded-with: 0.6 * (0.97 * 0.95)^1.0 = 0.6 * 0.9215 = 0.5529
        assert abs(rows_r[0]["strength"] - 0.6 * 0.97) < 1e-4
        assert abs(rows_e[0]["strength"] - 0.6 * (0.97 * 0.95)) < 1e-4


# -----------------------------------------------------------------------
# 5. Prune weak synapses (explicit pass)
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 5b. LTP synapse protection
# -----------------------------------------------------------------------


class TestLTPSynapseProtection:
    """Verify LTP protection counteracts decay for high-access atom synapses."""

    async def test_synapse_between_high_access_atoms_decays_less(
        self, storage: Storage
    ) -> None:
        """Synapse between two atoms with access_count >= 20 should barely decay.

        Both endpoints qualify for tier-20 (ltp_factor=0.1).  Geometric mean
        protection = sqrt(0.1 * 0.1) = 0.1.  Net decay = 0.97^0.1 ≈ 0.9970,
        so strength should be ≈ 0.6 * 0.9970 ≈ 0.5982 — well above the
        unprotected value of 0.582.
        """
        a1 = await _insert_atom(
            storage, content="frequently used atom 1", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
            storage, content="frequently used atom 2", created_at="2020-01-01 00:00:00"
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 25 WHERE id = ?", (a1,)
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 25 WHERE id = ?", (a2,)
        )
        syn_id = await _insert_synapse(
            storage, a1, a2, relationship="related-to", strength=0.6
        )
        ...
        assert len(rows) == 1
        # Unprotected decay: 0.6 * 0.97 = 0.582
        # Protected (tier-20 both): 0.6 * 0.97^0.1 ≈ 0.5982
        assert rows[0]["strength"] > 0.59  # Noticeably better than 0.582

    async def test_synapse_between_low_access_atoms_not_protected(
        self, storage: Storage
    ) -> None:
        """Synapse between atoms with access_count < 5 decays at the full base rate."""
        a1 = await _insert_atom(
            storage, content="rarely used atom 1", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
            storage, content="rarely used atom 2", created_at="2020-01-01 00:00:00"
        )
        # Default access_count = 0, well below min LTP tier of 5
        syn_id = await _insert_synapse(
            storage, a1, a2, relationship="related-to", strength=0.6
        )
        ...
        assert len(rows) == 1
        # Normal decay: 0.6 * 0.97 = 0.582
        assert abs(rows[0]["strength"] - 0.6 * 0.97) < 1e-4

    async def test_mixed_access_counts_uses_geometric_mean(
        self, storage: Storage
    ) -> None:
        """One high-access + one medium-access endpoint applies geometric mean protection.

        a1: access_count=25 → tier-20, ltp_factor=0.1
        a2: access_count=7  → tier-5,  ltp_factor=0.5
        geometric mean = sqrt(0.1 * 0.5) ≈ 0.2236
        Net decay = 0.97^0.2236 ≈ 0.9932 → strength ≈ 0.5959
        Result should be between full protection (≈0.5982) and no protection (0.582).
        """
        a1 = await _insert_atom(
            storage, content="high access atom", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
            storage, content="medium access atom", created_at="2020-01-01 00:00:00"
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 25 WHERE id = ?", (a1,)
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 7 WHERE id = ?", (a2,)
        )
        syn_id = await _insert_synapse(
            storage, a1, a2, relationship="related-to", strength=0.6
        )
        ...
        # Should be noticeably better than unprotected but less than fully protected
        assert strength > 0.582 + 0.005  # Better than no protection
        assert strength < 0.6  # But still some decay

    async def test_supersedes_not_protected(self, storage: Storage) -> None:
        """Supersedes synapses are excluded from LTP protection.

        The supersedes relationship has its own decay rules (very slow decay only
        for synapses older than 180 days).  A freshly-created supersedes synapse
        receives no decay at all in a single cycle, so its strength stays unchanged.
        """
        a1 = await _insert_atom(
            storage, content="atom 1", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
            storage, content="atom 2", created_at="2020-01-01 00:00:00"
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 25 WHERE id = ?", (a1,)
        )
        ...
        syn_id = await _insert_synapse(
            storage, a1, a2, relationship="supersedes", strength=0.6
        )
        ...
        assert len(rows) == 1
        # Supersedes synapse is < 180 days old → no decay applied and no LTP boost.
        # Strength should be essentially unchanged.
        assert abs(rows[0]["strength"] - 0.6) < 1e-4

    async def test_protection_details_in_result(self, storage: Storage) -> None:
        """ConsolidationResult.details includes a protect_ltp_synapses entry."""
        a1 = await _insert_atom(
            storage, content="atom 1", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
        ...
        await _insert_synapse(storage, a1, a2, relationship="related-to", strength=0.6)

        engine = _make_engine(storage)
        result = await engine.reflect()

        ltp_details = [
            d for d in result.details if d.get("action") == "protect_ltp_synapses"
        ]
        assert len(ltp_details) == 1
        assert ltp_details[0]["protected_count"] >= 1
        assert "base_decay_used" in ltp_details[0]

    async def test_one_low_access_endpoint_not_protected(
        self, storage: Storage
    ) -> None:
        """If only ONE endpoint has high access_count, the synapse is not protected.

        Both endpoints must qualify for the minimum LTP tier (access_count >= 5).
        """
        a1 = await _insert_atom(
            storage, content="high access atom", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
            storage, content="cold atom", created_at="2020-01-01 00:00:00"
        )
        await storage.execute_write(
            "UPDATE atoms SET access_count = 25 WHERE id = ?", (a1,)
        )
        # a2 keeps the default access_count = 0
        syn_id = await _insert_synapse(
            storage, a1, a2, relationship="related-to", strength=0.6
        )
        ...
        assert len(rows) == 1
        # No protection — a2 is below the minimum LTP tier
        assert abs(rows[0]["strength"] - 0.6 * 0.97) < 1e-4

    async def test_dry_run_counts_eligible_synapses(self, storage: Storage) -> None:
        """In dry-run mode, result includes the count of synapses that would be protected."""
        a1 = await _insert_atom(
            storage, content="atom 1", created_at="2020-01-01 00:00:00"
        )
        a2 = await _insert_atom(
        ...
        assert len(ltp_details) == 1
        assert "Would protect" in ltp_details[0]["note"]
        assert "1 synapses" in ltp_details[0]["note"]


# -----------------------------------------------------------------------
# 6. Merge near-duplicate atoms
# -----------------------------------------------------------------------
...
class TestMergeDuplicates:
    ...
    async def test_merge_identifies_duplicates(self, storage: Storage) -> None:
        ...
        # H-1: embed_batch is called once upfront; map embeddings back to
        # content so search_similar (now called with vectors) still works.
        _emb_lookup: dict[float, str] = {}
        ...
        async def mock_embed_batch(texts):
            ...
            for i, text in enumerate(texts):
                emb = [float(i + 1)] * 768
                _emb_lookup[float(i + 1)] = text
                result.append(emb)
            return result

        async def mock_search_similar(text_or_emb, k=10):
            content = text_or_emb
            if isinstance(text_or_emb, list):
                content = _emb_lookup.get(text_or_emb[0], "")
...
# -----------------------------------------------------------------------
# 7. Promote frequently accessed atoms
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 14. Feedback signals → importance adjustment
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 15. Episodic-to-semantic abstraction
# -----------------------------------------------------------------------
...
class TestAbstractExperiences:
    ...
    async def test_fact_confidence_scales_with_cluster_size(self, storage: Storage) -> None:
        ...
        assert fact_rows
        # M-3: log-scale — 5-member cluster → min(0.95, 0.5 + 0.15 * log1p(5)) ≈ 0.769
        import math
        expected = min(0.95, 0.5 + 0.15 * math.log1p(5))
        assert abs(fact_rows[0]["confidence"] - expected) < 1e-4
...
class TestTuneRetrievalWeights:
    ...
    async def test_nudges_confidence_weight_toward_good(self, storage: Storage) -> None:
        ...
        # confidence weight should have moved above the factory default 0.12
        assert saved["confidence"] > 0.12
    ...
    async def test_weights_clamped_to_max_drift(self, storage: Storage) -> None:
        ...
        default_confidence = 0.12  # factory default from RetrievalWeights
        max_drift = 0.30
        hi = default_confidence * (1 + max_drift)
        await storage.save_retrieval_weights(
            {"confidence": hi, "importance": 0.11, "frequency": 0.02,
             "recency": 0.08, "spread_activation": 0.25}
        )
...
# -----------------------------------------------------------------------
# 17. Consolidation ordering: decay runs BEFORE abstraction (B4)
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 18. Type-differentiated decay (B1)
# -----------------------------------------------------------------------
...
class TestTypeDifferentiatedDecay:
    ...
    async def test_custom_type_multipliers_respected(self, storage: Storage) -> None:
        ...
        # With multiplier 0.5 the skill decays much faster: effective_rate = 0.97 * 0.5 = 0.485
        # Experience uses 1.0: effective_rate = 0.97 * 1.0 = 0.97
...
class TestLTDSemanticExemption:
    ...
    async def test_ltd_skips_contradicts_synapses(self, storage: Storage) -> None:
        ...
        # Synapse may be subject to passive synapse decay (0.97 factor) but NOT LTD.
        ltd_details = [
            d for d in result.details
            if d.get("action") == "ltd" and d.get("synapse_id") == synapse_id
        ]
...
# -----------------------------------------------------------------------
# 20. Feedback for deleted / nonexistent atoms (A4)
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 21. Proportional LTD + batch weaken
# -----------------------------------------------------------------------
