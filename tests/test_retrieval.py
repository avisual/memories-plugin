...
class TestVectorSearchSeeds:
    ...
    async def test_seeds_from_search_similar(
        ...
        assert result.seed_count >= 1  # Vector + BM25 seeds
...
class TestSpreadingActivationSummation:
    ...
    async def test_convergent_activation_exceeds_single_path(
        ...
        With max-with-convergence, target gets max(a, b) + 0.1 * min(a, b).
        This preserves strong single-path signals while still rewarding
        atoms reachable via multiple routes.
        """
        ...
        # Max-with-convergence: max(a, b) + 0.25 * min(a, b).
        # Both paths contribute equal activation, so:
        #   expected = from_one + 0.25 * from_one = 1.25 * from_one
        expected_convergent = min(1.0, expected_from_one_seed + expected_from_one_seed * 0.25)
        assert actual > expected_from_one_seed + 1e-6, (
            f"Activation {actual:.4f} should exceed single-seed value "
            f"{expected_from_one_seed:.4f} — convergent paths must boost"
        )
        assert actual == pytest.approx(expected_convergent, abs=1e-6)
