...
class TestAssessNovelty:
    ...
    async def test_novel_when_no_existing_atoms(
        ...
        is_novel, similar_count = await learning_engine.assess_novelty("brand new topic")
    ...
    async def test_novel_when_below_threshold(
        ...
        is_novel, similar_count = await engine.assess_novelty("somewhat different content")
    ...
    async def test_not_novel_when_very_similar_and_confident(
        ...
        is_novel, similar_count = await engine.assess_novelty("Redis SCAN performance")
    ...
    async def test_novel_when_similar_but_low_confidence(
        ...
        is_novel, similar_count = await engine.assess_novelty("Redis SCAN info")
    ...
    async def test_empty_content_is_not_novel(
        ...
        is_novel_1, _ = await learning_engine.assess_novelty("")
        is_novel_2, _ = await learning_engine.assess_novelty("   ")
        assert is_novel_1 is False
        assert is_novel_2 is False
    ...
    async def test_embedding_error_assumes_novel(
        ...
        is_novel, similar_count = await learning_engine.assess_novelty("some content")
    ...
    async def test_custom_threshold(
        ...
        is_novel, _ = await engine.assess_novelty("Redis content", threshold=0.9)
        assert is_novel is True

        # With threshold=0.5, similarity 0.8 is above -> not novel (atom_a confidence = 0.95).
        is_novel, _ = await engine.assess_novelty("Redis content", threshold=0.5)
        assert is_novel is False
    ...
    async def test_deleted_closest_atom_is_novel(
        ...
        is_novel, _ = await engine.assess_novelty("Redis SCAN content")
        assert is_novel is True
