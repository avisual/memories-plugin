...
## Step 3: Retrieve atom details

For each atom ID from the recall result, call:
`mcp__memories__recall` with the atom's content as the query to get full details,
OR use `sqlite3 ~/.memories/memories.db "SELECT * FROM atoms WHERE id = <id>;"` directly.
Do NOT construct WHERE id IN (...) clauses manually from recalled IDs.
...
For each retrieved atom, look up its connections individually:

```sql
-- For a single retrieved atom ID, find its connections to other retrieved atoms
sqlite3 ~/.memories/memories.db "SELECT s.source_id, s.target_id, s.relationship, ROUND(s.strength, 3) as strength, SUBSTR(a1.content, 1, 60) as source_preview, SUBSTR(a2.content, 1, 60) as target_preview FROM synapses s JOIN atoms a1 ON a1.id = s.source_id JOIN atoms a2 ON a2.id = s.target_id WHERE s.source_id = <atom_id> OR s.target_id = <atom_id> ORDER BY s.strength DESC LIMIT 10;"
```

Alternatively, use `mcp__memories__pathway` with an atom ID to get the full connection graph.
