...
_NEWNESS_BONUS_WEIGHT = 0.08
"""Additional score weight for atoms created within the newness window.

Reduced from 0.15 — the newness bonus compresses the scoring range for
all atoms created within 7 days (they ALL get the same boost), which
reduces the discrimination power of vector/spread/BM25 signals.
0.08 still provides a meaningful recency advantage for new atoms while
freeing 0.07 effective weight for discriminating signals."""
...
_MIN_ATOMS_TARGET = 20
"""Minimum atoms to retain after min_score filtering.

If fewer atoms pass the configured ``min_score`` floor, the floor is
adaptively lowered so at least this many atoms reach the budget fitting
stage.  This prevents context starvation on hard queries where spreading
activation produces few high-scoring atoms."""
...
Controls the balance between top-ranked and lower-ranked atoms when
fusing vector and BM25 result lists.  Higher values give more weight
to lower-ranked results; 60 is the standard value in the literature."""
...
def _reciprocal_rank_fusion(
    ...
    """Fuse vector and BM25 seed rankings into a single score.

    Uses Reciprocal Rank Fusion which is scale-invariant — it works on
    rank positions rather than raw scores, so different score distributions
    from vector similarity and BM25 are handled correctly.

    Atoms found by both systems get a natural boost; atoms found by only
    one system receive a penalty rank for the missing system.

    Returns scores normalised to [0, 1].
    """
    # Build rank orderings (1-indexed, lower rank = better).
    ...
    vector_rank = {aid: rank for rank, aid in enumerate(vector_ranked, 1)}
    bm25_rank = {aid: rank for rank, aid in enumerate(bm25_ranked, 1)}

    all_candidates = set(vector_scores) | set(bm25_scores)

    fused: dict[int, float] = {}
    for aid in all_candidates:
        v_rank = vector_rank.get(aid, missing_rank)
        b_rank = bm25_rank.get(aid, missing_rank)
        fused[aid] = 1.0 / (k + v_rank) + 1.0 / (k + b_rank)

    # Normalise to [0, 1].
    if fused:
        max_score = max(fused.values())
        if max_score > 0:
            return {aid: score / max_score for aid, score in fused.items()}
    return fused
...
_FTS5_STOP_WORDS: frozenset[str] = frozenset({
    "the", "is", "at", "in", "on", "to", "of", "it", "if", "by",
    "as", "be", "do", "go", "he", "me", "my", "no", "so", "up",
    "we", "an", "am", "us", "was", "are", "has", "had", "did",
    "how", "who", "its", "can", "for", "but", "this", "that",
    "what", "when", "where", "which", "with", "from", "been",
    "have", "were", "does", "will", "would", "could", "should",
    "their", "there", "about", "them", "they",
})
"""Common English stop words excluded from FTS queries to reduce noise."""
...
def _sanitize_fts_query(query: str) -> str:
    ...
    technical abbreviations like "CI", "DB", "UI"), excludes FTS5 operator
    keywords and common stop words, then joins with OR so BM25 scores
    documents containing *any* query term — matching more terms scores higher.
    """
    words = [
        w for w in re.findall(r"[a-zA-Z0-9_]{2,}", query)[:20]
        if w.lower() not in _FTS5_KEYWORDS
        and w.lower() not in _FTS5_STOP_WORDS
    ]
...
class RetrievalEngine:
    ...
    async def recall(
        ...
        vector_scores, bm25_result = await asyncio.gather(
            self._vector_search(
        ...
        bm25_scores, bm25_extended = bm25_result

        # Reciprocal Rank Fusion: combine vector and BM25 rankings into
        # a unified seed score that is robust to different score scales.
        # Atoms found by both systems get boosted; single-source atoms
        # still enter but with appropriate discount.
        # Reference: Cormack, Clarke & Büttcher (2009).
        fused_seeds = _reciprocal_rank_fusion(
            vector_scores, bm25_scores, k=_RRF_K,
            missing_rank=self._cfg.seed_count * 3 + 1,
        )
        ...
        if session_atom_ids:
            for aid in session_atom_ids:
                if aid not in fused_seeds:
                    fused_seeds[aid] = _SESSION_PRIME_BOOST

        seed_count = len(fused_seeds)
        ...
        # Use bm25_extended (all FTS matches) instead of bm25_scores (just
        # seeds) so non-seed atoms containing query keywords still get a
        # BM25 signal in the composite score.  This improves MRR by ~0.03
        # on keyword-rich queries where evidence contains the answer keyword
        # but was not in the top-k BM25 seeds.
        # Seed-only BM25 — validated by A/B test (Test A J=0.786 vs Test B ~same).
        ...
        # Skip when a specific region was requested — all results are from
        # that region anyway, so the cap would just discard relevant atoms.
        cfg = get_config()
        diversity_cap = cfg.region_diversity_cap
        if diversity_cap > 0 and region is None:
        ...
        #
        # Adaptive: if fewer atoms pass the floor than _MIN_ATOMS_TARGET,
        # lower the floor so the budget fitting stage (not this filter)
        # decides what to keep.  This prevents context starvation on hard
        # queries where spreading activation produces few high-scoring atoms.
        ...
        if min_score > 0:
            ...
            if len(passing) < _MIN_ATOMS_TARGET and len(scored_atoms) >= _MIN_ATOMS_TARGET:
                sorted_scores = sorted(
                    (s for _, s, _ in scored_atoms), reverse=True,
                )
                adaptive_floor = sorted_scores[_MIN_ATOMS_TARGET - 1] * 0.95
                scored_atoms = [
                    (atom, score, bd)
                    for atom, score, bd in scored_atoms
                    if score >= adaptive_floor or atom.type == "antipattern"
                ]
            else:
                scored_atoms = passing
    ...
    async def _bm25_search(
        ...
        1. **Seed scores** -- the top *k* BM25 matches used for RRF fusion
           and spreading activation seeding.
        2. **Extended scores** -- ALL FTS matches (up to ``k * 5``), normalised
           to ``[0, 1]``.  Atoms beyond the top-*k* seeds still receive a
           BM25 score in the multi-factor ranking stage, so evidence atoms
           containing query keywords get a scoring boost even when they were
           not seeded.  This significantly improves MRR for keyword-rich
           queries (single-hop, temporal) where the evidence atom contains
           the answer keyword but ranks below the seed cut-off.
        ...
        -------
        tuple[dict[int, float], dict[int, float]]
            ``(seed_scores, extended_scores)`` where both are normalised
            to ``[0, 1]``.  ``seed_scores`` is a subset of
            ``extended_scores``.
        """
        ...
        for row in rows:
            ...
            if types is not None and atom.type not in types:
                continue
            extended[atom_id] = row["raw_score"]
            if len(seeds) < k:
        ...
        # Normalise raw BM25 scores to [0, 1] using the global max.
        max_score = max(extended.values())
        if max_score > 0:
            norm_seeds = {aid: score / max_score for aid, score in seeds.items()}
            norm_extended = {aid: score / max_score for aid, score in extended.items()}
        else:
            norm_seeds = {aid: 0.0 for aid in seeds}
            norm_extended = {aid: 0.0 for aid in extended}

        return norm_seeds, norm_extended
        ...
        for level in range(1, depth + 1):
            ...
            for atom_id in frontier:
                ...
                for neighbor_id, synapse in neighbors:
                    ...
                    # M-5: Per-hop linear decay — activation attenuates by a
                    # constant factor at each hop (Collins & Loftus 1975),
                    # not exponentially with depth.
                    # L-3: For inhibitory (contradicts) synapses, use
                    # type_weight=1.0 to avoid double-attenuation — the
                    # 0.7 type_weight would pre-attenuate the suppression
                    # signal before negation, weakening the intended effect.
                    effective_weight = 1.0 if synapse.relationship == "contradicts" else type_weight
                    neighbor_activation = (
                        current_activation
                        * synapse.strength
                        * effective_weight
                    ...
                    # Max-with-convergence: the strongest path dominates,
                    # but additional convergent paths add a 25% bonus.
                    # Raised from 10% — multi-hop questions with evidence
                    # spread across sessions benefit from convergence: when
                    # multiple session seeds all reach the same junction atom
                    # via different paths, it should rank noticeably higher.
                    # This preserves strong single-path signals (e.g. part-of
                    # chains) while differentiating atoms reachable via
                    # multiple routes (H1 refractory gates expansion only).
                    prev = activated.get(neighbor_id, 0.0)
                    if neighbor_activation > prev:
                        new_val = min(1.0, neighbor_activation + prev * 0.25)
                    else:
                        new_val = min(1.0, prev + neighbor_activation * 0.25)
                    activated[neighbor_id] = max(0.0, new_val)
