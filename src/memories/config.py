...
class SynapseTypeWeights:
    ...
    part_of: float = 0.85       # Sequential turns in conversation — primary traversal path
    related_to: float = 0.65    # Generic embedding similarity — raised from 0.5; hop1 activation +33% enables content-similar atom bridging
...
class RetrievalWeights:
    ...
    spread_activation: float = 0.25  # iter4 value — testing whether 0.32 helped or hurt
    recency: float = 0.08
...
class RetrievalConfig:
    ...
    seed_count: int = 25  # iter4 value — testing whether 35 caused context dilution
    spread_depth: int = 2  # iter4 value — depth-3 may introduce noise atoms
    decay_factor: float = 0.85  # per-hop linear decay; raised from 0.7 — hop2 part-of activation +47% (0.0283→0.0418 at fanout=8)
    min_activation: float = 0.01  # Lowered from 0.1 — enables hop2 part-of chains and cross-session traversal despite fanout normalization; min_score (0.18) is the real quality gate
    min_score: float = 0.18  # filter atoms below this composite score before budget fitting
...
class LearningConfig:
    ...
    auto_link_threshold: float = 0.72  # Lowered from 0.78 — broader connectivity for conversational atoms; STC + decay self-cleans weak links
    hebbian_increment: float = 0.12
    ...
    stc_tagged_strength: float = 0.30
    """Floor strength for new auto-linked related-to synapses under Synaptic
    Tagging and Capture.  Actual initial strength is
    ``max(stc_tagged_strength, similarity * stc_similarity_scale)`` so
    high-confidence matches start strong enough for spreading activation
    while low-confidence matches still need Hebbian reinforcement.
    Implements Frey & Morris (1997) synaptic tagging and capture."""

    stc_similarity_scale: float = 0.55
    """Scaling factor applied to embedding similarity to compute initial
    synapse strength for related-to links.  A similarity of 0.78 yields
    initial strength 0.429 — above the spreading activation propagation
    threshold (~0.405 for related-to with fanout normalization).  Lower
    similarities fall back to stc_tagged_strength floor."""
    ...
    rate_recall_alpha: float = 0.05
    """EMA learning rate for rate_recall() weight updates.
    Higher values adapt faster but can destabilize weights."""
...
class ConsolidationConfig:
    ...
    decay_min_interval_hours: int = 20
    """Minimum hours between decay cycles.

    Prevents graph destruction when consolidation runs multiple times/day
    via MCP tool calls.  The 3am launchd job always runs decay; daytime
    MCP-triggered consolidation skips the decay step."""
    ...
    distill_oversized: bool = True
    """Distill atoms whose content exceeds ``distill_max_chars`` during
    consolidation.  Uses a local Ollama model to summarise verbose atoms
    into concise, self-contained facts (~100 tokens).  Disabled when
    Ollama is unreachable (graceful fallback to truncation)."""

    distill_max_chars: int = 200
    """Content length threshold above which an atom is considered oversized
    and eligible for distillation during consolidation."""

    distill_batch_size: int = 100
    """Maximum number of oversized atoms to distill per consolidation cycle.
    Keeps consolidation time bounded (~1-2s per atom with Ollama)."""

    pattern_detection_enabled: bool = True
    """Auto-detect recurring error patterns in experience atoms and promote
    them to antipatterns during consolidation.

    When a cluster of ``pattern_min_cluster`` or more similar experiences
    all contain error/failure language, the system creates an antipattern
    from the most-accessed cluster member and links the others via
    ``warns-against`` synapses.  Set to ``False`` to disable."""

    pattern_min_cluster: int = 3
    """Minimum number of similar error experiences before auto-creating
    an antipattern.  Lower values are more aggressive; higher values
    require stronger corroboration before flagging a pattern."""

    pattern_similarity: float = 0.85
    """Embedding similarity threshold for clustering error experiences.
    Higher than ``abstraction_similarity`` (0.82) to avoid false positives
    — only very similar errors should be grouped."""

    pattern_min_age_days: int = 3
    """Error experiences must be at least this old before pattern detection
    considers them.  Prevents reacting to a single bad session."""

    pattern_max_per_cycle: int = 5
    """Cap on new auto-detected antipatterns per consolidation cycle."""

    skill_gen_min_atoms: int = 15
    """Minimum high-importance atoms (importance >= 0.6) in a project region
    before auto-generating SKILL.md during consolidation.

    Set to 0 to disable automatic skill generation entirely.
    """
...
@dataclass(frozen=True, slots=True)
class AntipatternClassificationConfig:
    """Parameters for automatic antipattern category classification."""

    enabled: bool = True
    similarity_voting_enabled: bool = True
...
class MemoriesConfig:
    ...
    skill_output_dir: str = ".claude/skills"
    """Directory (relative to cwd) where auto-generated SKILL.md files are written.

    Auto-skill generation writes ``{skill_output_dir}/{project}/SKILL.md``.
    Set to ``""`` to suppress file writes while still returning skill content.
    Override via ``MEMORIES_SKILL_OUTPUT_DIR`` environment variable.
    """

    retrieval: RetrievalConfig = field(default_factory=RetrievalConfig)
    learning: LearningConfig = field(default_factory=LearningConfig)
    consolidation: ConsolidationConfig = field(default_factory=ConsolidationConfig)
    antipattern_classification: AntipatternClassificationConfig = field(
        default_factory=AntipatternClassificationConfig
    )
