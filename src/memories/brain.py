...
# ---------------------------------------------------------------------------
# In-process LRU recall cache (constants only — cache lives on Brain instance)
# ---------------------------------------------------------------------------

_RECALL_CACHE_MAX = 256
...
def _recall_cache_key(
    ...
    """Compute a short hash key for recall cache lookups.

    All recall parameters are included so that calls differing only in
    budget_tokens, depth, or include_antipatterns produce distinct keys.
    MD5 is used instead of SHA-256 for speed -- this is an in-process LRU
    cache, not a security context.
    """
    payload = f"{query}|{region}|{sorted(types or [])}|{budget_tokens}|{depth}|{include_antipatterns}"
...
class Brain:
    ...
    def __init__(self, db_path: Path | None = None) -> None:
        ...
        self._recall_cache: OrderedDict = OrderedDict()

    def __getattr__(self, name: str) -> Any:
        """Lazy-init for attributes missing in test fixtures that bypass __init__."""
        if name == "_recall_cache":
            # Test fixtures using Brain.__new__() skip __init__ — initialise on demand.
            cache: OrderedDict = OrderedDict()
            object.__setattr__(self, "_recall_cache", cache)
            return cache
    ...
    async def remember(
        ...
        confidence:
            Initial confidence in ``[0, 1]`` (default ``1.0``).  Auto-captured
            atoms may use a lower initial confidence based on how many similar
            atoms already exist in the graph.
        ...
        # 0a. Content truncation cap — keep atoms concise.
        if len(content) > 2000:
            content = content[:1997] + "..."

        # Invalidate recall cache — a new atom may change recall results.
        ...
        if type == "antipattern":
            ...
            # 2b. Auto-classify antipattern category if not already tagged.
            if self._config.antipattern_classification.enabled:
                if not any(
                    t.startswith("category:") for t in (tags or [])
                ):
                    category = await self._learning.suggest_antipattern_category(
                        content, tags
                    )
                    tags = list(tags or [])
        ...
        # 4. Auto-link + detect supersession in parallel.
        ...
        # Both operations do independent embedding searches and write
        # non-overlapping synapse types, so they run safely in parallel.
        created_synapses, supersede_count = await asyncio.gather(
            self._learning.auto_link(atom.id),
            self._learning.detect_supersedes(atom.id),
        )
    ...
    async def recall(
        ...
        # M-4: Include session priming fingerprint in cache key so that
        # changes to the session atom set bust the cache.
        cache_key = _recall_cache_key(
            query, region, types, budget_tokens, depth, include_antipatterns,
        )
        cache_key = f"{cache_key}_{len(session_atom_ids)}"
        cached = self._recall_cache.get(cache_key)
        if cached is not None:
            cached_result, cached_at = cached
            if (time.monotonic() - cached_at) < _RECALL_CACHE_TTL:
                self._recall_cache.move_to_end(cache_key)
                logger.debug("Recall cache HIT for key %s", cache_key)
        ...
        self._recall_cache[cache_key] = (recall_result, time.monotonic())
        self._recall_cache.move_to_end(cache_key)
    ...
    async def forget(self, atom_id: int, hard: bool = False) -> dict[str, Any]:
        ...
        # Track manual deletion in atom_rejections for feedback loop.
        if success and self._storage is not None:
            try:
                source_proj = atom.region.removeprefix("project:") if atom.region else atom.region
                await self._storage.execute_write(
                    "INSERT INTO atom_rejections (content, reason, hook, source_project) "
                    "VALUES (?, 'manual_delete', 'forget', ?)",
                    (atom.content[:500], source_proj),
                )
            except Exception:
                pass  # Best-effort — don't fail forget() if tracking fails.
    ...
    async def amend(
        ...
        # Content truncation cap — keep atoms concise (matches remember()).
    ...
    async def get_hook_stats(self) -> dict[str, Any]:
        ...
        # All queries are independent reads — run them concurrently via
        # asyncio.gather to eliminate sequential thread-dispatch overhead.
        (
            rows_7d,
            rows_30d,
            rows_all,
            ...
            self._storage.execute(
                ...
                "WHERE created_at >= datetime('now', '-7 days') GROUP BY hook_type"
            ),
            self._storage.execute(
                "SELECT hook_type, COUNT(*) AS cnt FROM hook_stats "
                "WHERE created_at >= datetime('now', '-30 days') GROUP BY hook_type"
            ),
            self._storage.execute(
            ...
            self._storage.execute(
                ...
                "FROM hook_stats WHERE atoms_returned > 0 GROUP BY hook_type"
            ),
            self._storage.execute(
                "SELECT AVG(avg_score) AS avg FROM hook_stats "
                "WHERE avg_score IS NOT NULL"
            ),
            self._storage.execute(
                "SELECT AVG(CASE WHEN budget_total > 0 "
                "THEN CAST(budget_used AS REAL) / budget_total * 100 "
                "ELSE NULL END) AS pct FROM hook_stats WHERE budget_total > 0"
            ),
            self._storage.execute(
                "SELECT COUNT(DISTINCT CAST(j.value AS INTEGER)) AS unique_atoms, "
            ...
            self._storage.execute(
                "SELECT CAST(j.value AS INTEGER) AS atom_id, COUNT(*) AS cnt, "
                "a.content AS content, a.type AS type "
                "FROM hook_stats h, json_each(h.atom_ids) AS j "
                "JOIN atoms a ON a.id = CAST(j.value AS INTEGER) "
                "WHERE h.atom_ids IS NOT NULL "
                "GROUP BY atom_id ORDER BY cnt DESC LIMIT 10"
            ),
            self._storage.execute(
                "SELECT novelty_result, COUNT(*) AS cnt FROM hook_stats "
                "WHERE novelty_result IS NOT NULL GROUP BY novelty_result"
            ),
            self._storage.execute(
                "SELECT hook_type, AVG(latency_ms) AS avg_ms, "
                "MAX(latency_ms) AS max_ms FROM hook_stats GROUP BY hook_type"
            ),
            self._storage.execute(
                "SELECT strftime('%Y-W%W', created_at) AS week, "
                "AVG(avg_score) AS relevance, COUNT(*) AS invocations, "
                "AVG(atoms_returned) AS avg_atoms FROM hook_stats "
                "WHERE hook_type = 'prompt-submit' AND avg_score IS NOT NULL "
                "GROUP BY week ORDER BY week DESC LIMIT 8"
            ),
            self._storage.execute(
                "SELECT signal, COUNT(*) AS cnt FROM atom_feedback "
                "GROUP BY signal"
            ),
        )
        ...
        result["counts_7d"] = {r["hook_type"]: r["cnt"] for r in rows_7d}
        result["counts_30d"] = {r["hook_type"]: r["cnt"] for r in rows_30d}
        result["counts_all"] = {r["hook_type"]: r["cnt"] for r in rows_all}

        # Avg atoms returned per hook type.
        result["avg_atoms_returned"] = {
            r["hook_type"]: round(r["avg_atoms"], 1) for r in rows_avg_atoms
        }

        # Avg relevance score.
        result["avg_relevance_score"] = (
            round(rows_avg_score[0]["avg"], 4)
            if rows_avg_score and rows_avg_score[0]["avg"] is not None
            else None
        )

        # Budget utilisation.
        result["budget_utilisation_pct"] = (
            round(rows_budget[0]["pct"], 1)
            if rows_budget and rows_budget[0]["pct"] is not None
            else None
        )
