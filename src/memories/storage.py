...
CREATE INDEX IF NOT EXISTS idx_consolidation_log_action_time
    ON consolidation_log(action, created_at DESC);
...
CREATE TABLE IF NOT EXISTS hook_stats (
    ...
    hook_type TEXT NOT NULL CHECK(hook_type IN (
        ...
        'task-completed','notification','teammate-idle'
    )),
    project TEXT,
    query TEXT,
...
CREATE INDEX IF NOT EXISTS idx_feedback_unprocessed
    ON atom_feedback(atom_id) WHERE processed_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_synapses_bidirectional
    ON synapses(target_id, strength DESC) WHERE bidirectional = 1;
CREATE INDEX IF NOT EXISTS idx_atoms_source_project ON atoms(source_project) WHERE source_project IS NOT NULL;
"""
...
class Storage:
    ...
    def _run_migrations(self, conn: sqlite3.Connection) -> None:
        ...
        # Migration 12: Backfill NULL accessed_at in hook_session_atoms (v1.12.0)
        # Migration 7 added the accessed_at column but ALTER TABLE ADD COLUMN
        # with a non-constant DEFAULT does not backfill existing rows.  Any rows
        # that predated that migration still have NULL accessed_at, which causes
        # temporal Hebbian calculations to treat them as infinitely old (epoch 0).
        # Backfill from the atom's created_at as the best available approximation.
        # M-3: Guard with user_version to avoid a full-table scan on every startup
        # once the one-time migration has already been applied.
        ...
        if user_version < 12:
            ...
            if null_count > 0:
                conn.execute("""
                    UPDATE hook_session_atoms
                    SET accessed_at = (
                        SELECT created_at FROM atoms
                        WHERE atoms.id = hook_session_atoms.atom_id
                    )
                    WHERE accessed_at IS NULL
                """)
                log.info(
                    "Migration: Backfilled %d NULL accessed_at rows in hook_session_atoms",
                    null_count,
                )
            conn.execute("PRAGMA user_version = 12")

        # Migration 13: Create retrieval_weight_log table for rate_recall feedback (v1.13.0)
        row = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='retrieval_weight_log'"
        ).fetchone()
        if row is None:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS retrieval_weight_log (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            """)
            log.info("Migration: Created 'retrieval_weight_log' table")

        # Migration 15: Add teammate-idle to hook_stats CHECK constraint (v1.15.0)
        schema = conn.execute(
            "SELECT sql FROM sqlite_master WHERE name='hook_stats'"
        ).fetchone()
        if schema and "teammate-idle" not in schema[0]:
            conn.execute("ALTER TABLE hook_stats RENAME TO hook_stats_old")
            conn.execute("""
                CREATE TABLE hook_stats (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    hook_type TEXT NOT NULL CHECK(hook_type IN (
            ...
            """)
            conn.execute("INSERT INTO hook_stats SELECT * FROM hook_stats_old")
            conn.execute("DROP TABLE hook_stats_old")
            log.info("Migration: Added teammate-idle to hook_stats CHECK constraint")

        # Migration 14: Add partial index for bidirectional synapse traversal (v1.14.0)
        idx_rows = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_synapses_bidirectional'"
        ).fetchall()
        if not idx_rows:
            conn.execute(
                "CREATE INDEX idx_synapses_bidirectional "
                "ON synapses(target_id, strength DESC) WHERE bidirectional = 1"
            )
            log.info("Migration: Added partial index idx_synapses_bidirectional")

        # Migration 16: Create atom_rejections and rejection_patterns tables (v1.16.0)
        row = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='atom_rejections'"
        ).fetchone()
        if row is None:
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS atom_rejections (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    content TEXT NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_rejections_reason ON atom_rejections(reason);
                CREATE INDEX IF NOT EXISTS idx_rejections_created ON atom_rejections(created_at);
                CREATE INDEX IF NOT EXISTS idx_rejections_reason_created ON atom_rejections(reason, created_at);
            """)
            log.info("Migration: Created 'atom_rejections' table")
        ...
        if row is None:
            conn.executescript("""
                ...
                CREATE INDEX IF NOT EXISTS idx_rejection_patterns_reason ON rejection_patterns(reason);
            """)
            log.info("Migration: Created 'rejection_patterns' table")

        # Migration 17: Backfill source_project from region for project-scoped atoms
        rows = conn.execute(
            "SELECT COUNT(*) AS cnt FROM atoms"
            " WHERE source_project IS NULL"
            "   AND region LIKE 'project:%'"
            "   AND is_deleted = 0"
        ).fetchone()
        if rows and rows[0] > 0:
            conn.execute(
                "UPDATE atoms"
                " SET source_project = substr(region, 9)"
                " WHERE source_project IS NULL"
                "   AND region LIKE 'project:%'"
                "   AND is_deleted = 0"
            )
            log.info(
                "Migration 17: Backfilled source_project for %d atoms from region",
                rows[0],
            )

        # Migration 18: Create metadata table for time-proportional decay
        row = conn.execute(
        ...
        if row is None:
            ...
            """)
            log.info("Migration 18: Created 'metadata' table")
    ...
    async def get_metadata(self, key: str) -> str | None:
        """Return a metadata value by key, or None if not set."""
