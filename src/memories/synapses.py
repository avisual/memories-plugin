...
class SynapseManager:
    ...
    async def hebbian_update(
        ...
        position_weights:
            Optional mapping of atom ID to serial position weight (0.5–1.0).
            When provided, the Hebbian increment for new pairs is scaled by
            the mean of both atoms' weights (L-4: Murdock 1962 serial position
            effect — primacy/recency atoms get stronger associations).

        Returns
        -------
        tuple[int, int]
            ``(strengthened, created)`` — the number of existing synapses that
            were strengthened and the number of new synapses created.
        """
        ...
        for id_a, id_b in itertools.combinations(sorted(unique_ids), 2):
            ...
            if pair in existing_by_pair:
                ...
                # L-4: Serial position effect — scale by mean of both atoms'
                # position weights.  Primacy/recency atoms (weight ≈1.0) form
                # stronger associations; middle-of-session atoms (≈0.5) are
                # weaker.  Backward-compatible: defaults to 1.0 when absent.
                if position_weights:
                    w_a = position_weights.get(id_a, 1.0)
                    w_b = position_weights.get(id_b, 1.0)
                    pair_increment *= (w_a + w_b) / 2.0
        ...
        if strengthen_ids:
            ...
            # H-3 STC capture: tagged synapses that reached activated_count >= 1
            # should be restored to full potentiated strength (0.5), not left at
            # the weak BCM-nudged value (~0.29). Clear the tag — it's captured.
            # M-2: Changed from > 1 to >= 1 — first co-activation is sufficient
            # evidence of a real association; waiting for a second activation
            # delayed capture by one full consolidation cycle.
            await self._storage.execute_write(
                f"UPDATE synapses "
                f"SET strength = MAX(strength, 0.5), "
                f"    tag_expires_at = NULL "
                f"WHERE id IN ({id_ph}) "
                f"  AND tag_expires_at IS NOT NULL "
                f"  AND activated_count >= 1",
                tuple(strengthen_ids),
            )
