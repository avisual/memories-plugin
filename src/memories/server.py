...
async def remember(
    ...
    """Store a new memory atom in the brain.

    Auto-creates synaptic pathways to related existing memories.
    Persist discrete pieces of knowledge (roughly 50-200 tokens).
    The brain automatically finds and links related memories via embedding similarity.
    Use for facts, experiences, skills, preferences, insights, or antipatterns.

    QUALITY: Content must include specific technical details — file paths,
    function names, error messages, config values, or concrete facts.
    Generic observations, meta-commentary, and tool narration will be rejected.
    ...
    """
    try:
        # Quality gate — reject junk before any DB work.
        from memories.cli import _is_junk, _record_rejection
        is_junk, reason = _is_junk(content, atom_type=type)
        if is_junk:
            await _ensure_brain()
            await _record_rejection(
                _brain, content, reason,
                hook="mcp.remember",
            ...
            return {
                "status": "rejected",
                "reason": reason,
                "message": f"Content rejected by quality filter ({reason}). "
                           "Rephrase with specific technical details (file names, function names, "
                           "concrete facts) and retry.",
            }
...
async def recall(
    ...
    Given a natural-language query, finds the most relevant memories then
    follows synaptic connections outward (spreading activation) to surface
    contextually related knowledge and antipattern warnings.

    Results are budget-aware: fits as many relevant atoms as possible within
    the token budget, compressing when necessary. Be specific in queries for
    best results (e.g. "Redis key expiry handling" not just "Redis").
...
@mcp.tool()
async def rate_recall(session_id: str, rating: str) -> dict[str, Any]:
    """Rate the quality of memories recalled in a session (good/bad).

    Applies an exponential moving average update to retrieval weights.
    Good ratings nudge weights toward components that scored highest.
    Bad ratings nudge weights away from them.

    Note: The retrieval_weight_log table is a write-only audit trail.
    Weights are applied immediately to the retrieval_weights singleton row,
    which Brain.initialize() reads at startup. The log itself does not
    feed back into recall -- it exists for debugging and trend analysis only.

    Args:
        session_id: The session ID whose recall quality you are rating.
        rating: Either 'good' (memories were relevant) or 'bad' (memories
            were irrelevant or unhelpful).

    Returns:
        A dict with keys:
        - status: 'applied' on success
        - rating: The rating that was applied
        - weight_delta: Dict showing how each weight was adjusted
        - new_weights: The updated retrieval weights
    """
    import json as _json

    if rating not in ("good", "bad"):
        return {"error": "ValueError", "detail": "rating must be 'good' or 'bad'"}

    try:
        await _ensure_brain()

        # Read current retrieval weights.
        stored_w = await _brain._storage.load_retrieval_weights()
        if stored_w is None:
            from memories.config import RetrievalWeights as _RW
            _defaults = _RW()
            stored_w = {
                "confidence": _defaults.confidence,
                "importance": _defaults.importance,
                "frequency": _defaults.frequency,
                "recency": _defaults.recency,
                "spread_activation": _defaults.spread_activation,
            }

        # EMA update — learning rate from config.
        from memories.config import get_config
        alpha = get_config().learning.rate_recall_alpha
        delta: dict[str, float] = {}

        # Simple feedback: good -> increase spread_activation and confidence
        # weights; bad -> decrease them.
        if rating == "good":
            for key in ("spread_activation", "confidence"):
                nudge = alpha * (1.0 - stored_w[key])
                delta[key] = round(nudge, 6)
                stored_w[key] = min(0.5, stored_w[key] + nudge)
        else:
            for key in ("spread_activation", "confidence"):
                nudge = -alpha * stored_w[key]
                delta[key] = round(nudge, 6)
                stored_w[key] = max(0.01, stored_w[key] + nudge)

        # Round for clean storage.
        for key in stored_w:
            stored_w[key] = round(stored_w[key], 6)

        # Persist updated weights.
        await _brain._storage.save_retrieval_weights(stored_w)

        # Audit log only — not read back at recall time.
        # See retrieval_weights singleton for active weights.
        await _brain._storage.execute_write(
            """
            INSERT INTO retrieval_weight_log (session_id, rating, weight_delta)
            VALUES (?, ?, ?)
            """,
            (session_id, rating, _json.dumps(delta)),
        )
        ...
        return {
            ...
            "rating": rating,
            "weight_delta": delta,
            "new_weights": stored_w,
        }
    except Exception as exc:
        logger.exception("rate_recall failed")
        return _error_response(exc)
